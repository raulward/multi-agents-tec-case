...
def fetch_url(url: str, fetch_overrides: FetchOverrides) -> FetchResult:
    with requests.Session() as session:
        ...
        _validate_max_bytes(response.content, fetch_overrides.max_bytes)
        return FetchResult(
            url=url,
...
def _request_with_retries(session: requests.Session, url: str, timeout_s: float, retries: int) -> requests.Response:
    last_error: Exception | None = None
    total_attempts = retries + 1
    for attempt in range(1, total_attempts + 1):
        try:
            response = _request_once(session=session, url=url, timeout_s=timeout_s)
            _log_fetch_event(
...
def _request_once(session: requests.Session, url: str, timeout_s: float) -> requests.Response:
    response = session.get(url, timeout=timeout_s, allow_redirects=True)
    response.raise_for_status()
    return response


def _validate_max_bytes(content: bytes, max_bytes: int) -> None:
    if len(content) > max_bytes:
        raise ValueError(f"response exceeds max_bytes={max_bytes}")


def _log_fetch_event(step: str, **extra: object) -> None:
    event = {"step": step}
    event.update(extra)
